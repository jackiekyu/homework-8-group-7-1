...
def sterne_hypergeom_conf_interval(
        n,
        x,
        N,
        cl=0.975,
    ...
    assert method in ("clopper-pearson", "sterne",
                      "wang"), 'invalid method.'
...
def hypergeom_acceptance_region(n, G, N, alpha=0.05, randomized=False):
    """
    Helper function.
    ...
    #     # move the outcome into the acceptance region
    #     while p_tail > alpha and J:
    #         j = J.pop()
    #         p_tail -= pmf[j]
    #         I_list.append(j)
    #     return_val = I_list
